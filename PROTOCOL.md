...
| 27 | 2026-08-28 | §6.7, entry 14 | **The reproducibility check of entry 14 fails.** With deterministic algorithms forced (entry 25), the three b0/b3 pairs do not agree bit-for-bit. Entry 14's claim that identical configuration yields identical weights at every epoch is superseded: determinism holds for the *configuration* — seeds, cuDNN flags, TF32 and the Cauchy backend are set and recorded — but not for the *output*. The cause is not established here. | Measured after the campaign, as entry 14 prescribes. `git diff` over `src/` and `scripts/` between the two commits is empty, so the code was identical; seed, cache (`ptbxl_hr_fs100_notch0`), scaler and `numerics` all match, and `selection` only decides which checkpoint is kept after training. The checkpoint fingerprints differ, but that is by construction — `training_fingerprint` includes `run_id` to prevent cross-run resume — and carries no claim about weights. At epoch 0, `train_loss` differs by 1.1e-6 and `macro_all` by 3.2e-4; every row of `history.csv` differs, on all three seeds. Entry 14 names the places to look on this branch — seeding, the crop sampler, data ordering — and `num_workers` is excluded from the fingerprint on the stated ground that it does not touch results, which this measurement puts in question: eight workers, and batch ordering, are the first thing to check. `pykeops`, which compiles CUDA reductions outside the scope of `torch.use_deterministic_algorithms`, is a second candidate. Neither has been tested and neither is asserted. Scale, per the second half of entry 14: between two seeds of the same cell, epoch-0 `train_loss` differs by 7.2e-4 and `macro_all` by 1.8e-3, so residual run-to-run drift is roughly 650× smaller than seed-to-seed variation on the loss. Best epochs agree in five of the six seed × metric comparisons, the exception being `macro_all` at seed 2 (37 against 40) — the noisy criterion of entry 13, on a near-tied argmax. Consequence for §8: none of the results rest on bit-exactness. They rest on five seeds per cell and the paired bootstrap of §8.1, and this drift sits inside the internal null contrast of entry 22, which is the metre §8.2 uses and which is reported beside every estimate. Entry 14's second branch — the pairs measuring the run-to-run floor at fixed seed — is the one that holds, despite determinism having been forced. **First post-run entry in §10.** |
