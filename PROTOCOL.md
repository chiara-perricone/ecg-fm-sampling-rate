...
| 9 | 2026-08-26 | §6 Arms | Arm A/B non isola il solo prior sulle scale temporali: comprende anche la correzione di lunghezza applicata a C all'inizializzazione. | Il file S4 vendorato (state-spaces, commit 137e49d) usa `length_correction=True`, che moltiplica C per `I - dA^L` all'init, con `dA` funzione di `dt` e `L = window_seconds * fs`. L'init di C dipende quindi da `dt * L`. In Arm B `dt * L = dt_default * window_seconds * 100` e' costante fra rate; in Arm A scala con fs. Poiche' `dt * L` e `dt * fs` differiscono solo per la costante `window_seconds`, non sono separabili: A-B resta un confronto a un grado di liberta', ma la descrizione corretta e' "tutto cio' che dipende da dt espresso in passi", non "il prior sulle scale temporali". Verificato pre-run in `test_dt_times_length_is_constant_in_arm_b` e `test_arm_a_kernel_init_drifts_with_rate`. |